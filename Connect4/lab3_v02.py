...
def alpha_beta_negamax (board, depth, color, alpha, beta):
    ...
    for moves in get_all_next_moves(board):
        return_value=-1*alpha_beta_negamax (moves[1],depth-1, -1*color, -1*alpha, -1*beta)
...
def alpha_beta_search(board, depth,eval_fn,get_next_moves_fn=get_all_next_moves,is_terminal_fn=is_terminal):
    ...
    beta= float("inf")
    for moves in get_all_next_moves(board):
        evaluated_value= alpha_beta_negamax(moves[1], depth-1,1,alpha, beta)
...
#                                                   depth=8,
#                                                   eval_fn=focused_evaluate)
alphabeta_player = lambda board: alpha_beta_search(board, depth=4, eval_fn=new_evaluate)    #Earlier the depth was 8, but in project2 Update, change was made
